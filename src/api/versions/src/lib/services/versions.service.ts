...
> {
  ...
  async duplicate(originId: string) {
    ...
    const catItemsToCopy = await this.db.categoryItem.findMany({
      where: { versionId: id },
    });

    try {
      const versionCopy: VersionEntity = await this.create({
        name: name + ' copy',
        integrationId,
      });

      const newCatItems: CreateCategoryItemDto[] = catItemsToCopy.map(
        (item) => {
          const result: any = {
            ...item,
      ...
      await Promise.all(
        newCatItems.map((data) => this.db.categoryItem.create({ data })),
      );
      return this.db.version.findFirst({
        where: { id: versionCopy.id },
        include: { categoryItems: true },
      });
    } catch (e: any) {
